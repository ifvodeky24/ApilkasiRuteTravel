...
class LocalDataSource private constructor(private val travelDao: TravelDao) {
    ...
    fun getAllTrayek(): LiveData<List<TrayekEntity>> = travelDao.getAllTrayek()
    ...
    fun getAllKondisiJalanSearch(query: String): LiveData<List<KondisiJalanEntity>> =
        travelDao.getAllKondisiJalanSearch(query)
    ...
    fun insertTrayek(trayek: List<TrayekEntity>) =
        travelDao.insertTrayek(trayek)
